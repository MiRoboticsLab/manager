...
public:
  void Init()
  {
    ...
    led_excute_client_ = battery_capacity_info_node_->create_client<protocol::srv::LedExecute>(
    ...
    audio_play_client_ = battery_capacity_info_node_->create_client<protocol::srv::AudioTextPlay>(
      "speech_text_play", rmw_qos_profile_services_default, bc_callback_group_);
  }
...
private:
  ...
  bool QueryLed(bool is_control_led)
  {
    auto request_led = std::make_shared<protocol::srv::LedExecute::Request>();
    request_led->occupation = is_control_led;
    request_led->client = "bms";
    request_led->mode = 0x02;
    request_led->effect = 0x09;
    request_led->r_value = 0xFF;
    request_led->g_value = 0x32;
    request_led->b_value = 0x32;

    // 头灯
    request_led->target = 1;
    ...
    // 眼灯
    request_led->target = 3;
    request_led->effect = 0x30;
