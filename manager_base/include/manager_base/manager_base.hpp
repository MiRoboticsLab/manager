...
public:
  explicit ManagerBase(const std::string& name) : name_(name) {
...
public:
  ...
  bool RegisterRosHandler(rclcpp::Node::SharedPtr node_ptr, 
    bool need_self_check_server=true) {
    if(node_ptr == nullptr) {
      return false;
    }
    state_sub_ = node_ptr->create_subscription<ManagerStateMsg>("manager_set_state",
      rclcpp::SystemDefaultsQoS(), std::bind(&ManagerBase::StateCheckout, this, std::placeholders::_1));

    state_pub_ = node_ptr->create_publisher<ManagerStateMsg>("manager_set_state", rclcpp::SystemDefaultsQoS());

    std::string server_name = std::string("manager_init_") + GetName();
    if( need_self_check_server ) {
      init_server_ = node_ptr->create_service<ManagerInitSrv>(server_name,
      std::bind(&ManagerBase::ManagerServiceCall, this, std::placeholders::_1, std::placeholders::_2));
    }
...
private:
   ...
   */
  void ManagerServiceCall(const std::shared_ptr<ManagerInitSrv::Request> request,
  ...
  void BuildStateMap() {
    state_map_.insert(std::make_pair(system::ManagerState::kLowPower, std::bind(&ManagerBase::OnLowPower, this)));
    state_map_.insert(std::make_pair(system::ManagerState::kError, std::bind(&ManagerBase::OnError, this)));
    state_map_.insert(std::make_pair(system::ManagerState::kSuspend, std::bind(&ManagerBase::OnSuspend, this)));
    state_map_.insert(std::make_pair(system::ManagerState::kProtected, std::bind(&ManagerBase::OnProtected, this)));
    state_map_.insert(std::make_pair(system::ManagerState::kActive, std::bind(&ManagerBase::OnActive, this)));
  }
